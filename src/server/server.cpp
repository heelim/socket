		...
		for(i=0; i<num_user; i++) {
			if(FD_ISSET(client[i].sock, &readfds)) {
				...
				if (rbuf[0] == '/') {
					...
					server.tcp_send_msg(client[i].sock, wbuf);
				}
				else {
					for(int j=0; j<num_user; j++) {
						if(i==j) continue;
						if(client[i].room==client[j].room)	server.tcp_send_msg(client[j].sock, rbuf);
					}
				}
				printf("%s", rbuf);
			}
...
void direct_message(int i) {
	if (strlen(rbuf)<5) sprintf(wbuf, "you should specify receiver and type some message\n");
		...
		if (strlen(rbuf)<12) sprintf(wbuf, "you should type some message\n");
			...
			server.tcp_send_msg(rbuf[10]-'0', wbuf);
...
void add_client(int sock) {
	...
	server.tcp_send_msg(client_sock, client_id);
